...
DENS=D# this one can be adjusted for different reasons (porosity of packing vs porosity of material / increase time step (no gravity -> no real effect on the result)

#### default values for testing
intR=R # allows near neighbour interaction (can be adjusted for every packing / the bigger -> the more brittle / careful when intR is too large -> bonds can be created "over" particles) -> intR can be calibrated to reach a certain coordination number K (see calculation on line 115)
YOUNG=Y # this one controls the Young's modulus of the material
ALPHA=A # this one controls the material Poisson's ratio of the material
TENS=T # this one controls the tensile strength UTS of the material
COH=C # this one controls the compressive strength UCS of the material, more precisely, the ratio UCS/UTS (from my experience: COH should be >= to TENS, >= 10*TENS for competent materials like concrete)
FRICT=F # this one controls the slope of the failure envelope (effect mainly visible on triaxial compression tests)
